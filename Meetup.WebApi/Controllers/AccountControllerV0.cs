...
namespace Meetup.WebApi.Controllers
{
    [AllowAnonymous]
    [Route("v0/Account")]
    public class AccountControllerV0 : ControllerBase
    {
        ...
        public AccountControllerV0(MeetupDbcontext meetupDbcontext, IOptions<TokenOptions> jwtOptions)
        {
        ...
        public async Task<IActionResult> Register([FromBody]RegisterUserCommand command)
        {
            ...
            if (user.IsValid())
            {
                await _meetupDbcontext.Users.AddAsync(user);
                await _meetupDbcontext.SaveChangesAsync();
                return Ok();
            }
            else
            {
                return BadRequest(new { Messages = user.Notifications });
            }
        }
